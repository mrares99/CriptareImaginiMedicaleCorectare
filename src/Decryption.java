...
public class Decryption {
     ...
     * @param colorChannel Imaginea care se doreste a fi decriptata.
     * @param channel Canalul de culoare asupra caruia se efectueaza decriptarea.
     * @return Imaginea decriptata.
     */
    public BufferedImage doDecryption(int [][] matrix, BufferedImage colorChannel, String channel) {
        ...
        switch(channel){
            case "red":
                for (int j = -1; ++j < h; ) {
                     row=new ArrayList<>();
                    for (int i = -1; ++i < w; ) {
                        String bitRepresentaton=String.format("%8s", Integer.toBinaryString(((byte)((pixels[j * w + i] >> 16) & 0xff)) & 0xFF)).replace(' ', '0');
            ...
            case "green":
                for (int j = -1; ++j < h; ) {
                     row=new ArrayList<>();
                    for (int i = -1; ++i < w; ) {
                        String bitRepresentaton=String.format("%8s", Integer.toBinaryString(((byte)((pixels[j * w + i] >>  8) & 0xff)) & 0xFF)).replace(' ', '0');
                        for(int counter=-1;++counter<8;){
                            row.add(Integer.parseInt(String.valueOf(bitRepresentaton.charAt(counter))));
                        }
                    }
            ...
            case "blue":
                for (int j = -1; ++j < h; ) {
                     row=new ArrayList<>();
                    for (int i = -1; ++i < w; ) {
                        String bitRepresentaton=String.format("%8s", Integer.toBinaryString(((byte)((pixels[j * w + i]      ) & 0xff)) & 0xFF)).replace(' ', '0');
                        for(int counter=-1;++counter<8;){
                            row.add(Integer.parseInt(String.valueOf(bitRepresentaton.charAt(counter))));
                        }
                    }
    ...
    /** Efectueaza rotire circulara spre stanga.
            ...
            case "red":
                for(int i=-1;++i<nrRows;){
                    ...
                    for(int j=-1;++j<nrBitsInARow;){
                        ...
                        counter++;
                        if(counter==8){
                            byteRGB= (byte) Integer.parseInt(result.toString(),2);
                            result.delete(0,8);
                            counter=0;
